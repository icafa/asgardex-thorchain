...
import { Coin, PaginatedQueryTxs, BroadcastTxCommitResult, StdTxFee } from '../src/types'
...
describe('Client Test', () => {
  ...
  it('should init, should have right prefix', async () => {
    let prefix = thorClient.getPrefix()
    expect(prefix).toEqual('thor')

    thorClient.setNetwork('testnet')
    prefix = thorClient.getPrefix()
    expect(prefix).toEqual('tthor')
  })
  ...
  it('sends vault tx', async () => {
    ...
    const result = await thorClient.vaultTx({
  ...
  it('sends normal tx', async () => {
    ...
    const result = await thorClient.normalTx({
      addressTo: to_address,
      amount: '100',
      asset: 'thor',
    })
